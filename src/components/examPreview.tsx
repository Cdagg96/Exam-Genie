...
// PDF generation function that matches the preview exactly
function generateExamPDF(exam: ExamDoc) {
  const doc = new jsPDF();
  const pageWidth = doc.internal.pageSize.getWidth();
  const margin = 28; // ~1 inch margin to match your preview padding

  // Set initial y position
  let y = margin;

  // Name field at top left (matches your preview)
  doc.setFont("helvetica", "normal");
  doc.setFontSize(11);
  doc.text("Name: ________________", margin, y);
  y += 15;

  // Header - matches your preview styling
  doc.setFont("helvetica", "normal");
  doc.setFontSize(11);
  ...
  // Draw header border (matches your preview's border-b)
  doc.setDrawColor(200, 200, 200);
  doc.line(margin, y, pageWidth - margin, y);
  y += 20;

  // Instructions section - matches your preview styling
  doc.setDrawColor(200, 200, 200);
  ...
  sortedQuestions.forEach((q, index) => {
    ...
    if (q.type === "MC" && q.snapshot?.choices) {
      ...
      // Code box (matches the space in your preview)
      doc.setDrawColor(200, 200, 200);
